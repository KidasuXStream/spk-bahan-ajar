...
class RankingAdvancedExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithColumnWidths, WithTitle, ShouldAutoSize
{
    protected $prodiId;
    protected $prodiName;
    protected $sessionId;
    ...
    protected $urgensiFilter;

    public function __construct($prodiId = null, $sessionId = null, $statusFilter = null, $urgensiFilter = null)
    {
        $this->prodiId = $prodiId;
        $this->sessionId = $sessionId;
        $this->statusFilter = $statusFilter;
        $this->urgensiFilter = $urgensiFilter;
    ...
    public function collection()
    {
        ...
        // Apply urgensi filter
        if ($this->urgensiFilter) {
            $query->where(function ($q) {
                $q->where('urgensi_prodi', $this->urgensiFilter)
                    ->orWhere('urgensi_tim_pengadaan', $this->urgensiFilter)
                    ->orWhere('urgensi_institusi', $this->urgensiFilter);
            });
        }
    ...
    public function map($pengajuan): array
    {
        ...
        return [
            ...
            $pengajuan->ranking_position ?? $pengajuan->ranking ?? '-',
            $statusPrioritas,
            $pengajuan->status_pengajuan ?? 'Belum diproses',
            $sessionInfo,
            $keterangan
        ];
    }
    ...
    {
        $title = 'Ranking Advanced ' . $this->prodiName;
        if ($this->sessionName !== 'Semua Session') {
            $title .= ' - ' . $this->sessionName;
        }
        if ($this->statusFilter) {
            $title .= ' - Status: ' . ucfirst($this->statusFilter);
        }
        if ($this->urgensiFilter) {
            $title .= ' - Urgensi: ' . ucfirst($this->urgensiFilter);
        }
        return $title;
    }
